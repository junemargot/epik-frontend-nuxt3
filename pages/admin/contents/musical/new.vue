    ...
    <div class="contents-wrap">
      <section class="product-registration">
        ...
        <div class="input__item">
          ...
          <input type="text" v-model="title" name="title" ref="titleInput" placeholder="제목을 입력해주세요"
            autocomplete="off" />
        </div>
        <div class="input__item">
        ...
        <div class="input__item">
          ...
          <input type="text" v-model="runningTime" name="runningTime" placeholder="공연시간을 선택해주세요" autocomplete="off" />
        </div>
        <div class="input__item">
        ...
        <div class="input__item">
          ...
          <input type="file" name="img" id="imgInput" accept=".jpg, .jpeg, .png" @change="imgUploadHandler" />
        </div>
...
/*
서버에 추가로 전송해야하는 ref객체(반응형 데이터 객체)
- 에디터에 추가된 이미지 파일명(fileNames)
*/

/*
ref객체는 어디에 쓰이는가?
- 템플릿에서 바인딩
- 자바스크립트 코드에서 직접 상태를 읽고 수정
(데이터가 변경되면 자동으로 해당 데이터와 연관된 DOM이나 컴포넌트가 업데이트되도록하는데 사용)
*/
...
const images = ref([]); // 파일첨부한 이미지 미리보기를 위한 배열 ref객체
      ...
      async addImageBlobHook(blob, callback) {
        try {
          ...
          */
          ...
          formData.append('domain', "musical");
...
// 첨부 파일 업로드 핸들러
const fileList = ref([]); // *** 폼데이터에 담기 위해 파일객체를 순서대로 담을 ref배열 객체 
const imgUploadHandler = (event) => {
  /* 
   event.target.files란?
   <input type="file"> 요소에서 파일 선택 이벤트가 발생했을 때 
   자동으로 생성되는 FileList 객체를 나타냄
   (이 객체는 사용자가 선택한 파일들의 목록을 포함)
  */
...
const cancelHandler = () => {
  ...
  // 티켓과 예매처 필드 비어있는지 확인
  const areTicketsEmpty = ticketFields.value.every(ticket => !ticket.seat.trim() && !ticket.price.trim());
  const areLinksEmpty = links.value.every(link => !link.seat.trim() && !link.link.trim());
...
const submitHandler = async () => {
  ...
  try {
    ...
    const { data, error, status } = await useFetch(`${apiBase}/admin/musical`, {
      method: 'POST',
      body: formDatas,
    });
    ...
    if (status.value === 'success') {
      ...
      router.push({
        path: '/admin/contents/musical/' + data.value.id
      });
    } else {
...
<style lang="css">
@import url('/public/css/admin/contents/musical/new.css');
</style>