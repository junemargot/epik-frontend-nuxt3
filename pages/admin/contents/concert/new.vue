      ...
      <form action="#" method="POST" id="regForm" enctype="multipart/form-data">
        <section class="product-registration">
          ...
          <div class="input__item">
            ...
            <input type="text" v-model="title" ref="titleInput" name="title" placeholder="공연명을 입력해주세요"
              autocomplete="off" />
          </div>
          <div class="input__item">
          ...
          <div class="input__item split">
            ...
            <input type="text" v-model="startDate" id="startDateInput" name="daterange" placeholder="시작일을 입력해주세요"
              autocomplete="off" readonly />
            <div class="input__title">공연종료일</div>
            <input type="text" v-model="endDate" id="endDateInput" name="daterange" placeholder="종료일을 입력해주세요"
              autocomplete="off" readonly />
          </div>
          <div class="input__item">
      ...
      async addImageBlobHook(blob, callback) {
        try {
          ...
          */
          ...
          formData.append('domain', "concert");

          // post 요청
          // const response = await fetch(`${apiBase}/admin/musical-image/upload-temp`, {
          ...
          // 에디터에 이미지 추가 요청
          callback(`http://localhost:8080/api/v1/uploads/temp/concert/${fileName}`);
        } catch (error) {
...
const submitHandler = async () => {
  ...
  const pathsWithoutDomain = imagePaths.map(src => {
    return src.replace('http://localhost:8080/api/v1/uploads/temp/concert/', '');
  });
  ...
  ticketFields.value.forEach((ticket, index) => {
    formDatas.append(`concertTicketPrices[${index}].seat`, ticket.seat); // 좌석명
    formDatas.append(`concertTicketPrices[${index}].price`, ticket.price); // 가격
  });

  // links를 MusicalTicketOfficeDto로 매핑
  links.value.forEach((link, index) => {
    formDatas.append(`concertTicketOffices[${index}].name`, link.name); // 판매처 이름
    formDatas.append(`concertTicketOffices[${index}].link`, link.link);   // 판매처 URL
  });
  ...
  try {
    ...
    const { data, error, status } = await useFetch(`${apiBase}/admin/concert`, {
      method: 'POST',
      body: formDatas,
    });
    ...
    if (status.value === 'success') {
      ...
      router.push({
        path: '/admin/contents/concert/' + data.value.id
      });
    } else {
